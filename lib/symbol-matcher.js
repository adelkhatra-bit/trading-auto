...
const SYMBOL_VARIANTS = {
  'XAUUSD': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.005 },
      { name: 'tradingview', priority: 2, tolerance: 0.005 },
      { name: 'yahoo', priority: 3, tolerance: 0.008, fallback: 'GC=F' }
    ],
    expectedDecimals: 2,
    typicalRange: { min: 1800, max: 2200 }  // Price range for validation
  },
  'EURUSD': {
    canonical: 'EURUSD',
    type: 'forex',
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.003 },
      { name: 'tradingview', priority: 2, tolerance: 0.003 },
      { name: 'yahoo', priority: 3, tolerance: 0.005, fallback: 'EURUSD=X' }
    ],
    expectedDecimals: 5,
  ...
  'GBPUSD': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.003 },
      { name: 'tradingview', priority: 2, tolerance: 0.003 },
      { name: 'yahoo', priority: 3, tolerance: 0.005, fallback: 'GBPUSD=X' }
    ],
    expectedDecimals: 5,
  ...
  'USDJPY': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.004 },
      { name: 'tradingview', priority: 2, tolerance: 0.004 },
      { name: 'yahoo', priority: 3, tolerance: 0.006, fallback: 'USDJPY=X' }
    ],
    expectedDecimals: 3,
  ...
  'BTCUSD': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.01 },
      { name: 'tradingview', priority: 2, tolerance: 0.01 },
      { name: 'yahoo', priority: 3, tolerance: 0.02, fallback: 'BTC-USD' }
    ],
    expectedDecimals: 2,
  ...
  'ETHUSD': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.015 },
      { name: 'tradingview', priority: 2, tolerance: 0.015 },
      { name: 'yahoo', priority: 3, tolerance: 0.025, fallback: 'ETH-USD' }
    ],
    expectedDecimals: 2,
  ...
  'US500': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.005 },
      { name: 'tradingview', priority: 2, tolerance: 0.005 },
      { name: 'yahoo', priority: 3, tolerance: 0.01, fallback: '^GSPC' }
    ],
    expectedDecimals: 1,
  ...
  'NAS100': {
    ...
    sources: [
      { name: 'mt5', priority: 1, tolerance: 0.005 },
      { name: 'tradingview', priority: 2, tolerance: 0.005 },
      { name: 'yahoo', priority: 3, tolerance: 0.01, fallback: '^IXIC' }
    ],
    expectedDecimals: 1,
 ...
 * @param {string} canonical - Canonical symbol
 * @param {object} availablePrices - { mt5: price, tradingview: price, yahoo: price }
 ...
 * @param {object} backendData - { mt5: { symbol, price }, yahoo: { symbol, price } }
...
function matchSymbolWithPriceValidation(tvSymbol, tvPrice, backendData) {
  ...
  // Step 2: Collect available prices from all sources
  const availablePrices = {};
  if (backendData.mt5?.price) availablePrices.mt5 = backendData.mt5.price;
  if (backendData.tradingview?.price) availablePrices.tradingview = backendData.tradingview.price;
  if (backendData.yahoo?.price) availablePrices.yahoo = backendData.yahoo.price;
