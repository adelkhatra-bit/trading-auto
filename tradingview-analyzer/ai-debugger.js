// ai-debugger.js v2.0 — Diagnostic IA Complet (Système MT5 + Backend + Extension Integration)
'use strict';
...
const AIDebugger = (() => {
      ...
      chrome.runtime.sendMessage({ type: 'GET_STATE' }, (resp) => {
        ...
        resolve({
          ...
          background: {
            ...
            mt5Connected: bgState.mt5Connected,
  ...
  async function buildDiagnosticPrompt(fullState, endpoints) {
    const lines = [
      '═══════════════════════════════════════════════════════════',
      'DIAGNOSTIC EXTENSION TRADING AUTO - MT5 ARCHITECTURE',
      '═══════════════════════════════════════════════════════════',
      '',
      ...
      `MT5 Bridge actif: ${fullState.background.mt5Connected ? '✅ YES' : '❌ NO'}`,
      ...
      '',
      '───────────────────────────────────────────────────────────',
      'ENDPOINTS BRIDGE (http://127.0.0.1:4000)',
      '───────────────────────────────────────────────────────────'
    ];
  ...
  function displayDiagnostic(result) {
    ...
    if (result.error) {
      ...
      if (parsed.cause) {
        html += `<div class="diagnostic-section">
          <strong>🔍 Cause Probable:</strong><br>
          ${parsed.cause}
        </div>`;
      }
      ...
      if (parsed.file) {
        html += `<div class="diagnostic-section">
          <strong>📄 Fichier Concerné:</strong><br>
      ...
      if (parsed.missing) {
        html += `<div class="diagnostic-section">
          <strong>⚙️ Branchement Manquant:</strong><br>
          ${parsed.missing}
        </div>`;
      }

      if (parsed.fix) {
        html += `<div class="diagnostic-section fix">
      ...
      if (parsed.codeSnippet) {
        html += `<div class="diagnostic-section code">
          <strong>💻 Code:</strong><br>
          <pre>${parsed.codeSnippet}</pre>
        </div>`;
      }

      if (parsed.raw) {
        html += `<div class="diagnostic-section raw">
