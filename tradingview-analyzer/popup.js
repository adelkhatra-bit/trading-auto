...
function handleNoData() {
  ...
  if (typeof state.coach === 'object') {
    state.coach.decision = 'NO DATA';
    state.coach.status = 'NO DATA';
  } else {
    state.coach = { decision: 'NO DATA', status: 'NO DATA' };
  }
  // UI
  var priceEl = document.getElementById('headPrice');
  if (priceEl) priceEl.textContent = 'NO DATA';
  var chartEl = document.getElementById('chart-container');
  if (chartEl) chartEl.innerHTML = '<div style="color:#f87171;font-weight:bold;padding:20px;">NO DATA</div>';
  var coachEl = document.getElementById('coachText');
  if (coachEl) coachEl.textContent = 'NO DATA';
  ['M1','M5','M15','H1'].forEach(tf => {
    var t = document.getElementById('tfc-' + tf + '-t');
    if (t) t.textContent = 'NO DATA';
    var s = document.getElementById('tfc-' + tf + '-s');
    if (s) s.textContent = 'NO DATA';
  });
  var sig = document.getElementById('signalText');
  if (sig) sig.textContent = 'NO DATA';
  var analysis = document.getElementById('analysisText');
  if (analysis) analysis.textContent = 'NO DATA';
  var diag = document.getElementById('diagLine');
  if (diag) diag.textContent = 'NO DATA';
}
...
async function pollLiveData() {
  try {
    ...
    // Données valides : update normal
    state.price = data.price;
    state.candles = data.candles;
    // ... (autres updates UI si besoin)
  } catch (e) {
    handleNoData();
  }
}
// Remplacer ou compléter le polling live par pollLiveData dans l'init/bind principal
