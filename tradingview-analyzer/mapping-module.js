// mapping-module.js — Gère recherche/sauvegarde correspondances MT5
...
const MappingModule = {
    ...
    try {
      ...
      if (data.ok && data.mappings) {
        data.mappings.forEach(m => {
          this._savedMappings[m.userInput] = m.mt5Symbol;
        });
  ...
  // ── RESOLVE SYMBOL (recherche intelligente MT5) ─────────────────────
  ...
  saveMapping: async function(userInput, mt5Symbol) {
    ...
    try {
      const resp = await fetch(`${this._apiBase}/mapping/save`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userInput: userInput.toUpperCase(), mt5Symbol }),
        signal: AbortSignal.timeout(5000)
      });
      ...
      if (data.ok) {
        // Update local cache
        this._savedMappings[userInput.toUpperCase()] = mt5Symbol;
        console.log(`[MAPPING] Saved: ${userInput} → ${mt5Symbol}`);
      }
  ...
  getSavedMappings: function() {
    ...
    for (const [user, symbol] of Object.entries(this._savedMappings)) {
      list.push({ userInput: user, mt5Symbol: symbol });
    }
    return list;
  },
